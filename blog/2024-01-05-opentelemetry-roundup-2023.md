...
1. How to setup opentelemetry at scale: The current otel documentation have focus on sending logs/traces/metrics data from individual application to the a backend. But when someone is deploying opentelemetry in their organizations - they need understanding of what are best practices to deploy it. e.g. 
    ...
    - Should telemetry data be sent directly to otel collector or should there be a queueing mechanism like Kafka infront ( link to our Kafka article)
...
- **New tools coming up leveraging OpenTelemetry data format**
 ...
 Tools like Tracetest, Traceloop & OpenLLMetry were released which leverage common data. We think the standardization of data formats will lead to an ecosystem of tools which go beyond observability into other areas like testing, performance, security, etc.
